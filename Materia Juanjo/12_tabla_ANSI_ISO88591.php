...
<html lang="es">
 ...
 <!--   <meta charset="UTF-8"> -->
    ...
    <title>Tabla ANSI en ISO-8859-1</title>
    <!--
    ...
    */  
    ...
    for($i=0;$i<26;$i++){//Cada iteración escribe una fila
        ...
        for($j=0;$j<10;$j++){//Una celda en cada iteración
            $dato=10*$i+$j;
            //Si no está en rango, pongo un blanco
            $dato=(($dato<32)||($dato>255))?"32":$dato;
           //Si está entre 128 y 160 están libres, Lo quito
            $dato=($dato>127 && $dato<161)?"32":$dato;
            /*
            $dato=($dato<128)?htmlentities(chr($dato)):chr($dato);
            print "<td>$dato</td>";
            */
            print "<td>".(($dato<128)?htmlentities(chr($dato)):chr($dato))."</td>";
        }
        print "\n</tr>\n";  
    }   
